...
-- Robust data loading (No-require fallback using hosted orbital-strike-data.json)
local MissileData = {}
...
pcall(function()
    local raw = game:HttpGet("https://raw.githubusercontent.com/picksov/Roblox/refs/heads/main/orbital-strike-data.json")
...
local function runAutoBuild()
    ...
    local success, err = pcall(function()
            ...
            for x = -halfX + scanMargin, halfX - scanMargin, stepX do
                ...
                for z = -halfZ + scanMargin, halfZ - scanMargin, stepZ do
                    ...
                    if precisionBuildEnabled then
                        ...
                        -- Speed mode: fire and forget, yield every 12 fires
                        if canPlaceAt(worldCF, geom.size, base) then
                            pcall(function() ReplicatedStorage.Events.PlaceMissile:FireServer(name, worldCF) end)
                            fires = fires + 1
                            invRemaining = invRemaining - 1
                            bumpPlaced()
                            if fires % 12 == 0 then task.wait(0.01) end
                        end
                    end
                end
            end
        ...
        if autoBuildMissilesToggleEnabled and cachedPlaced < 250 then
                    ...
                    for z = -halfZ, halfZ, 1.8 do
                        ...
                        if precisionBuildEnabled then
                            ...
                            if canPlaceAt(worldCF, geom.size, base) then
                                ...
                                placedCount = placedCount + 1
                                bumpPlaced()
                                if gapFires % 12 == 0 then task.wait(0.01) end
                            end
                        end
                        invLeft = invLeft - 1
                    end
                                    ...
                                    if defenseShieldTypes[shieldName] then
                                        ...
                                        if placeCount > 0 then
                                            for k = 1, placeCount do
                                                ...
                                                needed = needed - 1
                                                task.wait(1.1) -- Debounce yield to prevent rate-limit (shields require a 1s server-side cooldown)
                                            end
                                        end
