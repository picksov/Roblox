...
warn("[Syphon] Scanning for Adonis...")
    ...
    local function hookDebugInfo(orig)
        ...
        pcall(function()
            local realOrig = hookfunction(orig, newcclosure(function(target, what, ...)
    ...
    if getType(iscclosure) == "function" then
        ...
        origCc = hookfunction(origCc, newcclosure(function(f, ...)
    ...
    origHook = hookfunction(origHook, newcclosure(function(f, wrapper)
        if f == detected then
...
-- Must run BEFORE armBypass() which uses task.wait()
...
CombatTab:CreateSection("Target Selector")

local selectedPlayerLabel = CombatTab:CreateLabel("Target Player: None")
...
CombatTab:CreateSection("System Status")

local statusParagraph = CombatTab:CreateParagraph({
    Title = "SYSTEM MONITOR",
    Content = "Status: IDLE\nTarget Component: None\nTarget HP: - / -"
})

CombatTab:CreateSection("Prioritization Mode")

modeDropdownUI = CombatTab:CreateDropdown({
...
CombatTab:CreateSection("Target Priority & Cycling")

local autoCycleToggleUI = CombatTab:CreateToggle({
...
local targetPriorityDropdownUI = CombatTab:CreateDropdown({
...
CombatTab:CreateSection("Priority Queue")
...
priorityInputUI = CombatTab:CreateInput({
...
CombatTab:CreateSection("Firing Commands")

autoFireToggleUI = CombatTab:CreateToggle({
...
salvoSizeInputUI = CombatTab:CreateInput({
...
local efficientModeToggleUI = CombatTab:CreateToggle({
...
local balancedFireToggleUI = CombatTab:CreateToggle({
...
CombatTab:CreateSection("🔥 Blatant — Rapid Fire")
...
local blatantToggleUI = CombatTab:CreateToggle({
...
CombatTab:CreateSection("Weapon Checklist")
...
weaponDropdownUI = CombatTab:CreateDropdown({
...
CombatTab:CreateSection("Auto-Build Missiles")
...
local precisionToggleUI = CombatTab:CreateToggle({
...
autoBuildToggleUI = CombatTab:CreateToggle({
...
autoBuildMissilesDropdownUI = CombatTab:CreateDropdown({
...
local autoSpinLuckyToggleUI = EconomyTab:CreateToggle({
...
EconomyTab:CreateSection("Rewards & Quests Auto-Claim")

autoClaimRewardsToggleUI = EconomyTab:CreateToggle({
...
local autoClanToggleUI = EconomyTab:CreateToggle({
...
BuildingTab:CreateSection("Defense Mode")

local defenseToggleUI = BuildingTab:CreateToggle({
...
local defenseShieldsDropdownUI = BuildingTab:CreateDropdown({
...
BuildingTab:CreateSection("Auto-Repair")

autoRepairToggleUI = BuildingTab:CreateToggle({
...
local hpBarsToggleUI = MiscTab:CreateToggle({
...
local missileAlertToggleUI = MiscTab:CreateToggle({
...
local missileAlertThresholdInputUI = MiscTab:CreateInput({
...
MiscTab:CreateSection("AFK Mode")
...
afkToggleUI = MiscTab:CreateToggle({
...
local afkFpsLimitInputUI = MiscTab:CreateInput({
    Name = "AFK FPS Limit",
    PlaceholderText = "30",
    RemoveTextAfterFocusLost = false,
    Callback = function(Text)
...
local function saveProfile(profileName)
    ...
    if not makefolder or not writefile then
        Rayfield:Notify({Title = "Error", Content = "File APIs not available!", Duration = 3})
        return
    end
...
local function loadProfile(profileName)
    ...
    local loadOk, loadErr = pcall(function()
        ...
        if autoCycleToggleUI then pcall(function() autoCycleToggleUI:Set(autoCycleEnabled) end) end
        if targetPriorityDropdownUI then pcall(function() targetPriorityDropdownUI:Set(targetPriorityMode) end) end
        if efficientModeToggleUI then pcall(function() efficientModeToggleUI:Set(efficientModeEnabled) end) end
        if balancedFireToggleUI then pcall(function() balancedFireToggleUI:Set(balancedFireEnabled) end) end
        if blatantToggleUI then pcall(function() blatantToggleUI:Set(blatantAutoEnabled) end) end
        if precisionToggleUI then pcall(function() precisionToggleUI:Set(precisionBuildEnabled) end) end
        if autoSpinLuckyToggleUI then pcall(function() autoSpinLuckyToggleUI:Set(autoSpinLucky) end) end
        if autoClanToggleUI then pcall(function() autoClanToggleUI:Set(autoClanClaimEnabled) end) end
        if defenseToggleUI then pcall(function() defenseToggleUI:Set(defenseModeEnabled) end) end
        if defenseShieldsDropdownUI then pcall(function() defenseShieldsDropdownUI:Set(getKeysFromMap(defenseShieldTypes)) end) end
        if hpBarsToggleUI then pcall(function() hpBarsToggleUI:Set(hpOverlayEnabled) end) end
        if missileAlertToggleUI then pcall(function() missileAlertToggleUI:Set(missileAlertEnabled) end) end
        if missileAlertThresholdInputUI then pcall(function() missileAlertThresholdInputUI:Set(tostring(missileAlertThreshold)) end) end
        if afkFpsLimitInputUI then pcall(function() afkFpsLimitInputUI:Set(tostring(afkFpsLimit)) end) end
        if autoRejoinToggleUI then pcall(function() autoRejoinToggleUI:Set(autoRejoinEnabled) end) end
        if afkToggleUI then pcall(function() afkToggleUI:Set(afkModeEnabled) end) end
        if uiToggleKeybindUI then pcall(function() uiToggleKeybindUI:Set("RightControl") end) end

        Rayfield:Notify({ Title = "Success", Content = "Loaded profile: " .. profileName, Duration = 3 })
    end)
    if not loadOk then
        Rayfield:Notify({ Title = "Error", Content = "Failed to load profile: " .. tostring(loadErr), Duration = 4 })
    end
end
...
local autoRejoinToggleUI = SettingsTab:CreateToggle({
...
do -- scoping block to stay under Luau 200-local register limit
...
task.spawn(function()
    ...
    while true do
        ...
        -- Lucky spin — independent of black market toggle and ship presence
...
-- auto-rejoin section (inside scoping block started above)
-- ──────────────────────────────────────────────────────────
