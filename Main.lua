...
local customFireCount = 50 -- missiles per single fire button
...
MiscTab:CreateButton({
    ...
    Callback = function()
        ...
        local ok, err = pcall(function()
            ...
            writefile("OrbitalStrikeTemplates/" .. templateName .. ".json", HttpService:JSONEncode(layout))
        end)
        if ok then
            Rayfield:Notify({ Title = "Saved", Content = templateName .. " (" .. total .. " items)", Duration = 3 })
        else
...
MiscTab:CreateButton({
    ...
    Callback = function()
        local path = "OrbitalStrikeTemplates/" .. templateName .. ".json"
        if not isfile or not isfile(path) then
            Rayfield:Notify({ Title = "Error", Content = "Template not found: " .. templateName, Duration = 2.5 })
            return
        end
        ...
        -- Phase 1: Clear all using hammer
        local hammer = bp:FindFirstChild("Hammer") or (char and char:FindFirstChild("Hammer"))
        if not hammer then
            Rayfield:Notify({ Title = "Error", Content = "Hammer not found in backpack!", Duration = 2.5 })
            return
        end
        pcall(function() hammer.Parent = char end)
        task.wait(0.1)

        local foldersToClear = {"PlacedMissiles", "PlacedBuildings", "PlacedTurrets", "PlacedShields"}
            ...
            if folder then
                for _, item in ipairs(folder:GetChildren()) do
                    if item and item.Parent then
                        pcall(function()
                            ReplicatedStorage.Events.PickupMissile:FireServer(item)
                        end)
                    end
                end
            end
        end
        Rayfield:Notify({ Title = "Clearing", Content = "Waiting for server...", Duration = 2 })
        task.wait(2)
        pcall(function() hammer.Parent = bp end)

        -- Phase 2: Place from template
        local function placeItems(list, remote)
            if not list then return end
            for _, item in ipairs(list) do
                local tool = bp:FindFirstChild(item.name) or (char and char:FindFirstChild(item.name))
                if tool then
                    ...
                    if hum then
                        ...
                        task.wait(0.1)
                        pcall(function()
                            remote:FireServer(item.name, CFrame.new(item.x, item.y, item.z))
                        end)
                    end
                end
            end
        ...
        placeItems(layout.buildings, ReplicatedStorage.Events.PlaceBuilding)
        placeItems(layout.turrets, ReplicatedStorage.Events.PlaceBuilding)
        placeItems(layout.shields, ReplicatedStorage.Events.PlaceShield)
        placeItems(layout.missiles, ReplicatedStorage.Events.PlaceMissile)
        ...
        Rayfield:Notify({ Title = "Done", Content = "Rebuilt " .. total .. " items from " .. templateName, Duration = 4 })
    end,
})
