-- ══════════════════════════════════════════════════════════
-- SECURITY LAYER 0: Anti-Hook Function Snapshotting
-- ══════════════════════════════════════════════════════════
local _HttpGet = game.HttpGet
local _HttpService = game:GetService("HttpService")
local _JSONDecode = _HttpService.JSONDecode
local _UrlEncode = _HttpService.UrlEncode
...
local _apiBytes = {104, 116, 116, 112, 115, 58, 47, 47, 97, 112, 105, 46, 112, 97, 110, 100, 97, 117, 116, 104, 46, 99, 111, 109, 47, 118, 49, 47, 112, 117, 98, 108, 105, 99, 47, 118, 97, 108, 105, 100, 97, 116, 101}
local _API = _ds(_apiBytes)

-- Encoded SERVICE_ID (same bytes as loader)
local _sidBytes = {111, 114, 98, 105, 116, 97, 108}
local _SID = _ds(_sidBytes)

-- Encoded key filename: OrbitalStrikeKey.txt
local _kfBytes = {79,114,98,105,116,97,108,83,116,114,105,107,101,75,101,121,46,116,120,116}
local _KF = _ds(_kfBytes)

-- ══════════════════════════════════════════════════════════
-- SECURITY LAYER 1: Session Token Validation Gate
-- Verifies the loader created a valid session token
-- ══════════════════════════════════════════════════════════
local _session = _getgenv().__OSH_SESSION
local _sigFn = _getgenv().__OSH_SIG
...
local function _validateSession()
    ...
    -- Verify token is not older than 5 minutes (loader should have just created it)
    if _ostime() - _session.t > 300 then return false end
...
if not _validateSession() then return end

-- Clean up session globals after validation (don't leave them accessible)
_getgenv().__OSH_SIG = nil
_getgenv().__OSH_SECRET = nil

-- ══════════════════════════════════════════════════════════
-- SECURITY LAYER 2: Sentinel Check Infrastructure
-- Silent inline validation calls scattered throughout the script
-- ══════════════════════════════════════════════════════════
local _sentinelOk = true

local function _sv()
    -- Silent verification: read saved key and check with PandaAuth
    local _k
    _pcall(function()
        if _isfile and _isfile(_KF) then _k = _readfile(_KF) end
    end)
    if not _k or _k == "" then _sentinelOk = false return end
    local _hw
    _pcall(function() _hw = _gethwid() end)
    if not _hw then _sentinelOk = false return end
    local _u = _API .. "?" .. "service=" .. _UrlEncode(_HttpService, _SID) .. "&key=" .. _UrlEncode(_HttpService, _k) .. "&hwid=" .. _UrlEncode(_HttpService, _hw)
    local s, r = _pcall(function() return _HttpGet(game, _u) end)
    if s and r then
        local ds, d = _pcall(function() return _JSONDecode(_HttpService, r) end)
        if ds and d and d.success == true then return end
    end
    _sentinelOk = false
end

-- ══════════════════════════════════════════════════════════
-- SECURITY LAYER 3: Periodic Background Re-Validation
-- Re-checks key with PandaAuth every 4 minutes
-- ══════════════════════════════════════════════════════════
task.spawn(function()
    task.wait(60) -- First check after 1 minute
    while true do
        _sv()
        if not _sentinelOk then
            -- Silently corrupt critical variables to break the script naturally
            task.wait(math.random(5, 15))
            _pcall(function()
                _getgenv().__OSH_SESSION = nil
            end)
            return
        end
        task.wait(240) -- Re-check every 4 minutes
    end
end)
...
local function runAutoBuild()
    ...
    if not _sentinelOk then return 0 end -- sentinel #1
    isBuilding = true
    local placedCount = 0
...
local function getMoney()
    if not _sentinelOk then return 0 end -- sentinel #3
...
local function purchaseCheckedInStockItems()
    ...
    if not _sentinelOk then return end -- sentinel #4
...
task.spawn(function()
    ...
    local _sc5 = 0 -- sentinel #5 counter
    while true do
        _sc5 = _sc5 + 1; if _sc5 % 200 == 0 then task.spawn(_sv) end
        if not _sentinelOk then task.wait(9e9) end
        if blatantAutoEnabled and activeTargetPlayer and _sentinelOk then -- sentinel #10 gate
            local now = os.clock()
...
task.spawn(function()
    ...
    local _sc6 = 0 -- sentinel #6 counter
    while true do
        _sc6 = _sc6 + 1; if _sc6 % 150 == 0 then task.spawn(_sv) end
        if not _sentinelOk then task.wait(9e9) end
        local shouldFire = isAutoFiring or burstRemaining > 0

        if shouldFire and activeTargetPlayer and _sentinelOk then -- sentinel #12 gate
...
-- ──────────────────────────────────────────────────────────
task.spawn(function()
    local _sc7 = 0 -- sentinel #7 counter
    while true do
        _sc7 = _sc7 + 1; if _sc7 % 50 == 0 then task.spawn(_sv) end
        if not _sentinelOk then autoBuyEnabled = false end
...
task.spawn(function()
    task.wait(0.3)
    if not _sentinelOk then return end -- sentinel #8
...
task.spawn(function()
    task.wait(0.5)
    if not _sentinelOk then return end -- sentinel #9
...
task.spawn(function()
    local lastAlert = 0
    task.spawn(_sv) -- sentinel #11 on defense thread init
    while true do
        if not _sentinelOk then defenseModeEnabled = false end
