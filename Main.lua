-- picksov Orbital Strike Command
...
makewritable = makewritable or (setreadonly and function(t) setreadonly(t, false) end) or function() end
fireproximityprompt = fireproximityprompt or (firesignal and function(p) firesignal(p.Triggered)) or function() end
getconnections = getconnections or get_signal_cons or function() return {} end
if not getloadedmodules then getloadedmodules = getmodules or get_loaded_modules or function() return {} end end
if not hookfunction then hookfunction = newcclosure and function(f,w) return newcclosure(w) end or function(f,w) return w end end
...
local itemRaw = HttpService:JSONDecode(game:HttpGet("https://raw.githubusercontent.com/picksov/Roblox/refs/heads/main/ItemsData.json"))
local function _bd(arr, fm)
    local r = {}
    for _, it in ipairs(arr) do
        if not it.blackMarket then
            local e = {}; for jk, lk in pairs(fm) do e[lk] = it[jk] end
            r[it.name] = e
        end
    end
    return r
end
local MissileData = _bd(itemRaw.missiles, {price="Price",damage="Damage",isCannon="IsCannon",health="Health",stock="Stock",radius="Radius",reload="Reload",buildTime="BuildTime"})
local BuildingData = _bd(itemRaw.buildings, {price="Price",income="Income",health="Health",stock="Stock",buildTime="BuildTime",repairTime="RepairTime",repairCost="RepairCost"})
local DefenseData = _bd(itemRaw.defenses, {price="Price",damage="Damage",health="Health",stock="Stock",buildTime="BuildTime",reload="Reload",barrels="Barrels",repairCost="RepairCost",repairTime="RepairTime",type="Type"})

-- ==================== Adonis Anti-Cheat Bypass ====================
setthreadidentity(8)
local bypassArmed = false
local bypassNotified = false
task.spawn(function()
    local getType = typeof or type
    if getType(filtergc) ~= "function" or getType(hookfunction) ~= "function" or getType(newcclosure) ~= "function" then
        bypassArmed = true return
    end
    local useUp = (getType(debug.setupvalue) == "function" and getType(debug.getupvalues) == "function")
    warn("[Syphon] Scanning for Adonis...")
    local Query = {Constants = {" - On Xbox", " - On mobile", "_"}, IgnoreExecutor = true}
    local d = nil
    while not d do d = filtergc("function", Query, true); if d and getType(d) == "function" then break end; d = nil; task.wait(0.5) end
    warn("[Syphon] Detected found")
    local sf, kf
    if useUp then local uv = debug.getupvalues(d); sf = uv[3]; kf = uv[9] end
    local sig = {}
    for _, fl in ipairs({"s","l","a","n"}) do local ok, v = pcall(debug.info, d, fl); if ok then sig[fl] = v end end
    local function hdi(orig)
        if getType(orig) ~= "function" then return end
        pcall(hookfunction, orig, newcclosure(function(t, w, ...)
            if t == d or t == kf then local r = {}; for i=1,#w do local f = w:sub(i,i); if sig[f] then r[#r+1] = sig[f] end end; return unpack(r) end
            return orig(t, w, ...)
        end))
    end
    hdi(debug.info)
    if getType(getrenv) == "function" then hdi(getrenv().debug.info) end
    if getType(debug.getinfo) == "function" then hdi(debug.getinfo) end
    if getType(iscclosure) == "function" then
        local oc = iscclosure
        pcall(hookfunction, oc, newcclosure(function(f, ...) if f == kf then return false end return oc(f, ...) end))
    end
    local oh = hookfunction
    pcall(hookfunction, oh, newcclosure(function(f, w) if f == d then return newcclosure(function() return task.wait(9e9) end) end return oh(f, w) end))
    if useUp and sf and getType(sf) == "function" then
        pcall(debug.setupvalue, d, 3, newcclosure(function() return nil end))
        if kf and getType(kf) == "function" then pcall(debug.setupvalue, d, 9, newcclosure(function() return nil end)) end
    else
        pcall(hookfunction, d, newcclosure(function() return task.wait(9e9) end))
    end
    pcall(function()
        for _, ch in ipairs(game:GetService("ReplicatedStorage"):GetChildren()) do
            if ch:IsA("RemoteEvent") and ch:FindFirstChild("__FUNCTION") then
                for _, cn in ipairs(getconnections(ch.OnClientEvent)) do pcall(function() cn:Disconnect() end) end
            end
        end
    end)
...
-- ==================== Bypass Notification (Rayfield) ====================
pcall(function()
    local tempWin = Rayfield:CreateWindow({
        Name = "Orbital Strike",
        LoadingTitle = "Loading...",
        LoadingSubtitle = "Preparing bypass",
        ConfigurationSaving = { Enabled = false },
    })
    local tempTab = tempWin:CreateTab("Status", "lock")
    tempTab:CreateSection("Please wait")
    Rayfield:Notify({ Title = "Orbital Strike", Content = "Bypass loading... please wait", Duration = 10 })
    while not bypassArmed do task.wait(0.3) end
    tempWin:Destroy()
end)

-- ==================== Bypass Gate ====================
while not bypassArmed do task.wait(0.5) end
...
if not task or not task.wait then
    ...
    getgenv().task = { wait = _wait, spawn = _spawn, delay = function(t, fn) _spawn(function() _wait(t) fn() end) end }
end

-- HttpService loaded from preamble
...
task.spawn(function()
    ...
    local buildingData = nil; buildingData = BuildingData
    local defenseData = nil; defenseData = DefenseData
